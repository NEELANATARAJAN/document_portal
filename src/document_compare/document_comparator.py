...
class DocumentComparatorLLM:
    def __init__(self):
        ...
        log.info("DocumentComparatorLLM initialized with model, parser, prompt and chain.", model=self.llm)
    ...
    def compare_documents(self, combined_docs: str) -> pd.DataFrame:
        try:
            ...
            log.error(f"Error in compare documents: {e}")
    ...
    def _format_response(self, response: list[dict]) -> pd.DataFrame:
        try:
            ...
            log.info("Response formatted into DataFrame successfully.", dataframe=df)
            return df
